...
use lsp::{WithTimestamp, Timestamp, InputState, LspContext, signal::{MappedSignal, Latch, ComputedLeveledSignal, LivenessSignal}};
...
fn main() {
    ...
    while let Some(moment) = ctx.next_event(&mut state) {
        ...
        println!("{} {} {:?}", moment.timestamp(), liveness_signal_output, state);
    }
