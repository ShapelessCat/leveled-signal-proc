...
mod lsp_app_combined {
    lsp_codegen::include_lsp_ir!(lsp_main @ "../lsdl/examples/app_combine.json");
}
fn main() {
    use lsp_playtime::*;
    ...
    let mut instr = lsp_runtime::instrument::InstrumentDataLogicRunningTime::default();
    lsp_main(input_iter, |m| {
        println!("{}", serde_json::to_string(&m).unwrap());
        Ok(())
    }, &mut instr).expect("Lsp Main Error");
    println!("{}", instr);
}
