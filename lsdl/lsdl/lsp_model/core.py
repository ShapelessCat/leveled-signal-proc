...
class LeveledSignalProcessingModelComponentBase(
    ...
    def add_metric(
        ...
        interval_metric_name: Optional[str] = None,
    ) -> Self:
...
__strict_and_reserved_rust_keywords = {
    *__config["strict"].values(),
    *__config["reserved"].values(),
}
...
def _validate_rust_identifier(identifier: str) -> None:
    ...
    if (
        not re.match(regex, identifier)
        or identifier in __strict_and_reserved_rust_keywords
    ):
...
class SignalBase(LeveledSignalProcessingModelComponentBase, ABC):
    ...
    def has_been_true(self, duration=-1) -> "SignalBase":
        ...
        return LevelTriggeredLatch(
            data=Const(True), control=self, forget_duration=normalize_duration(duration)
        )
    ...
    def prior_different_value(
        self, scope: Optional["SignalBase"] = None
    ) -> "SignalBase":
        return self.prior_value(self, scope)

    @final
    def prior_value(
        self, clock: Optional["SignalBase"] = None, scope: Optional["SignalBase"] = None
    ) -> "SignalBase":
        ...
        return (
            builder.build()
            .annotate_type(f"({ty}, {ty})")
            .map(bind_var="(ret, _)", lambda_src="ret.clone()")
    ...
    def _bin_op(self, other, op, typename=None) -> "SignalBase":
        ...
        if isinstance(other, SignalBase):
            ...
            bind_var = "lhs"
            is_cmp_string = (
                op in SignalBase.__CMP_OP and self.get_rust_type_name() == "String"
            )
            bind_var_in_use = (
                f"{bind_var}.as_str()" if is_cmp_string else f"*{bind_var}"
            )
            ret = SignalMapper(
                bind_var,
    ...
    def add_metric(
        ...
        measurement_config().add_metric(
            key, self.peek(), typename, need_interval_metric, interval_metric_name
        )
        return self
...
class MeasurementBase(LeveledSignalProcessingModelComponentBase, ABC):
    ...
    def add_metric(
        ...
        measurement_config().add_metric(
            key, self, typename, need_interval_metric, interval_metric_name
        )
        return self
    ...
    @final
    def combine(
        self, bind_var0: str, bind_var1: str, lambda_src: str, other: "MeasurementBase"
    ) -> "MeasurementBase":
