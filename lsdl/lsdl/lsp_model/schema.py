...
class SignalDataTypeBase(LeveledSignalProcessingModelComponentCore, ABC):
    ...
    def schema_ir(self) -> dict[str, Any]:
        if self._schema_ir is None:
            raise NotImplementedError("This field should have a non-None value")
...
class Object(SignalDataTypeBase):
    def __init__(self, name: Optional[RustCode] = None):
        super().__init__(rust_type=name or self.__class__.__name__)
        self._schema_ir = {"type": "object"}
...
@final
class String(SignalDataTypeBase, WithLiteralValue):
    def __init__(self):
...
@final
class Bool(SignalDataTypeBase, WithLiteralValue):
    def __init__(self):
        super().__init__("bool")
        self._schema_ir = { "type": "boolean" }
...
@final
class Integer(SignalDataTypeBase, WithLiteralValue):
    def __init__(self, signed=True, width=32):
        ...
        self._schema_ir = {
            ...
            "x-rust-type": {
                "crate": "std",
                "version": "*",
                "path": f"std::primitive::{self.get_rust_type_name()}"
            }
        }
...
@final
class Float(SignalDataTypeBase, WithLiteralValue):
...
@final
class CStyleEnum(SignalDataTypeBase, WithLiteralValue):
    def __init__(self, python_str_enum: Type[LspEnumBase]):
        ...
        self._schema_ir = {
            "type": "string",
            "enum_variants": self.str_enum_type.variants_info(),
        }
...
@final
class Vector(SignalDataTypeBase, WithLiteralValue):
    def __init__(self, element_type: SignalDataTypeBase):
        super().__init__(f"Vec<{element_type.get_rust_type_name()}>")
        self._element_type = element_type
        self._schema_ir = { "type": "array" }  # TODO: "x-rust-type"???

    @override
    def render_rust_const(self, val, _need_owned: bool = True) -> RustCode:
        if isinstance(self._element_type, WithLiteralValue):
...
class MappedInputMember(_InputMember):
    ...
    # TODO: Can we move `parse` and `starts_with` to `SignalBase`??? or `MeasureBase` if we can change the return type to `MeasureBase`???
    def parse(
