...
attempt_event = SignalFilterBuilder(input.video_event).filter_values("c3.video.attempt").build_clock_filter()
...
player_state = SignalFilterBuilder(input.video_event).filter_values("c3.video.buffering", "c3.video.play").build_value_filter()

# So the buffering time is simply defined as the duration of the player_state being "buffering"
(player_state == "c3.video.buffering").measure_duration_true().add_metric("bufferingTime")

# First we can identify if user is seeking
seeking = (input.raw_event == "seek_forward") | (input.raw_event == "seek_backward")

# Then we can define the connection induced buffering as following:
#   1. The player_state is "buffering"
#   2. The player_state has been "play" before
#   3. The user is seeking within 5s
((player_state == "c3.video.buffering") &\
    has_been_true(player_state == "c3.video.play") &\
