...
is_buffering.measure_duration_true().scope(input_signal.session_signal).add_metric(
    "buffering_time"
)

# - Initial buffering time
has_been_playing = (
    StateMachineBuilder(input_signal.session_id.clock(), input_signal.player_state)
    .transition_fn(
        f"""
...
(~has_been_playing & is_buffering).measure_duration_true().scope(
    input_signal.session_signal
...
# ev - seek time
(input_signal.ev == const.EV_SEEK_START).measure_duration_true().scope(
    input_signal.session_signal
