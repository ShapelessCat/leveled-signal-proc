...
use lsp_component::{processors::{SignalMapper, LivenessChecker, Latch, DurationOfPreviousLevel, StateMachine, Accumulator}, measurements::{DurationSinceBecomeTrue, ChangeSinceCurrentLevel}};
use lsp_runtime::{WithTimestamp, Timestamp, InputState, LspContext, signal::SingnalProcessor, measurement::Measurement};
...
fn main() {
    ...
    let mut p_e_state_machine = StateMachine::<String, u32, _, Timestamp>::new(0, |&prev_state, input| match prev_state {
        0 if input == "P" => 1,
    ...
    while let Some(moment) = ctx.next_event(&mut state) {
        ...
        if moment.should_take_measurements() {
            ...
            println!("{} {}", moment.timestamp(), if user_active_time > 0 { pe_time as f64 /user_active_time as f64 } else { 0.0 });
